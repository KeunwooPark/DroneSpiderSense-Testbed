...
function createLogicalMap(
  ...
  while (backtrackingStack.length > 0) {
    ...
    const adjacentNode = lastNode.getRandomUnVisitiedAdjacentNode(nodeMap);
...
class Node {
  ...
  public getRandomUnVisitiedAdjacentNode(nodeMap: Node[][]): Node | null {
    const adjacentNodes = this.getAdjacentNodes(nodeMap);
    const unvisitedAdjacentNodes = adjacentNodes.filter(
      (node) => !node.visited
    );
...
class Edge {
  ...
  getPhysicalPositionsToRemove(): Vector2[] {
    const positions = [];
    positions.push(
      new Vector2(this.from.getPhysicalRow(), this.from.getPhysicalCol())
    );
    positions.push(
      new Vector2(this.to.getPhysicalRow(), this.to.getPhysicalCol())
    );

    if (this.direction === "up") {
      positions.push(
        new Vector2(this.from.getPhysicalRow() + 1, this.from.getPhysicalCol())
      );
      ...
      positions.push(
        new Vector2(this.from.getPhysicalRow(), this.from.getPhysicalCol() + 1)
      );
  ...
  getTargetPosition(): Vector2 | null {
    if (this.to.isTarget()) {
      ...
      return new Vector2(
        this.from.getPhysicalRow(),
        this.from.getPhysicalCol()
      );
    }
    return null;
  }
...
function generatePhysicalMapFromEdges(
  edges: Edge[],
  width: number,
  height: number
): number[][] {
...
export default function MapGenerator(props: IMapGeneratorProps) {
  ...
  return (
    <>
      ...
      <button className="btn btn-primary" onClick={clickGenerateMap}>
        generate
      </button>
      <Map2DVisualizer
        height={height}
